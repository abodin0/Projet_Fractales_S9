#include "Convergence_GPU_mme.hpp"
...
Convergence_GPU_mme::Convergence_GPU_mme() : Convergence("GPU_double_mme")
{

}


Convergence_GPU_mme::Convergence_GPU_mme(ColorMap* _colors, int _max_iters) : Convergence("GPU_double_mme")
{
    colors    = _colors;
...
Convergence_GPU_mme::~Convergence_GPU_mme( ){
     cudaError_t cudaStatus = cudaDeviceReset();
     free(hostTab);
     free(deviceTab);
}

void Convergence_GPU_mme::updateImage(const long double _zoom, const long double _offsetX, const long double _offsetY, const int IMAGE_WIDTH, const int IMAGE_HEIGHT, sf::Image& image)
{
    ...
    kernel_updateImage_GPU_mme<<<grid, block>>>(zoom, offsetX, offsetY, IMAGE_WIDTH, IMAGE_HEIGHT, deviceTab, max_iters);
