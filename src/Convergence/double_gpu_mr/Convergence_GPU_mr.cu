#include "Convergence_GPU_mr.hpp"
...
Convergence_GPU_mr::Convergence_GPU_mr() : Convergence("GPU_double_mr")
{

}


Convergence_GPU_mr::Convergence_GPU_mr(ColorMap* _colors, int _max_iters) : Convergence("GPU_double_mr")
{
    colors    = _colors;
...
Convergence_GPU_mr::~Convergence_GPU_mr( ){
     cudaError_t cudaStatus = cudaDeviceReset();
     free(hostTab);
     free(deviceTab);
}

void Convergence_GPU_mr::updateImage(const long double _zoom, const long double _offsetX, const long double _offsetY, const int IMAGE_WIDTH, const int IMAGE_HEIGHT, sf::Image& image)
{
    ...
    kernel_updateImage_GPU_mr<<<grid, block>>>(zoom, offsetX, offsetY, IMAGE_WIDTH, IMAGE_HEIGHT, deviceTab, max_iters);
